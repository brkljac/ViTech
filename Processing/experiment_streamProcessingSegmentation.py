...
# stream processing semantic segmentation - test script
...
send_to_file = 1            #(1-send to file, 0-do not write on disk)
send_to_port = 0            #(1-send to port, 0-do not stream over port)
...
send_unprocessed = 0        #(1-yes, 0-no), in addition to processed vidout
...
output_image_stream = 1              #(1-yes, 0-no)
...
if USE_TensorRT:
    ...
    model_base_name = "fcn_resnet101"				
    model_relative_path = "models/torch/fcn_resnet101/"
    engine_input_width = 520           # trt input width
    engine_input_height= 520           # trt input height
    ...
    USE_FP16 = False                   # floating point precision
    if USE_FP16:
        fp_precision = 16
    else:
        fp_precision = 32
    ...
    def postprocess_engineOutput(input, output, temp1, temp2, temp3, interpolationMethod=cv2.INTER_NEAREST, alpha=0.6):
        # based on input (segmentation mask) create pseudocolor image, resize to output resolution and blend using alpha value
        num_classes = 21    # determined by the inference model
        # create a color palette
        palette = np.array([2 ** 25 - 1, 2 ** 15 - 1, 2 ** 21 - 1])
        colors = np.array([palette*i%255 for i in range (num_classes)]).astype("uint8")
        # create look-up table
        lut = np.zeros((256, 1, 3), dtype='uint8')
        lut[0:21, 0, 0] = colors[:,0]
        lut[0:21, 0, 1] = colors[:,1]
        lut[0:21, 0, 2] = colors[:,2]
        # cast single channel image to integer and apply colormap
        temp1[:,:] = cv2.applyColorMap(input.astype('uint8'), lut)
        width, height, _ = output.shape      
        # resize to original frame size
        temp2[:,:] = cv2.resize(temp1, (height, width), interpolation=interpolationMethod)
        # create alpha channel, transparency mask
        (_, temp3[:,:, 0]) = cv2.threshold(temp2[:,:, 2], 0, maxval=alpha, type=cv2.THRESH_BINARY)
        temp3[:,:, 1] = temp3[:,:, 0]
        temp3[:,:, 2] = temp3[:,:, 0]
        # apply the mask to foreground and background
        output[:,:] = cv2.multiply(1.0 - temp3, output.astype('float32'))
        temp2[:,:] = cv2.multiply(temp3, temp2)
        # blended image
        output[:,:] = cv2.add(output, temp2.astype('uint8'))
...
while True:
    ...
    if USE_TensorRT:
        if not engine_initialization_finished and frameNumber == 1:
            ...
            temp1_post = np.zeros((engine_input_height, engine_input_width, engine_input_depth), dtype=frame.dtype)
            temp2_post = np.zeros(frame.shape, dtype='float32')
            temp3_post = np.zeros(frame.shape, dtype='float32')
    ...
    if USE_TensorRT:
        ...
        if not context.execute_async_v2(bindings=engine_bindings, stream_handle=engine_stream.handle):
            print("Model execution did not succeed, please check context.execute_async_v2() call ...")
            continue
        ...
        # postprocess the results and send downstream (in the given case frame will be blended with segmentation mask)
        postprocess_engineOutput(engine_output, frame, temp1_post, temp2_post, temp3_post)
    ...
    if show_camout_on_screen:
        ...
        #cv2.imshow("Processing engine", frame)
